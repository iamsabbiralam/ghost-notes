...
<body class="bg-[#0f172a] text-slate-200 min-h-screen font-sans">
    <div class="max-w-7xl mx-auto py-12 px-6">
                ...
                <div class="relative export-menu" x-data="{ open: false }">
                    ...
                    <div x-show="open" @click.away="open = false" x-transition
                        ...
                        <button onclick="window.print()"
                            class="w-full text-left block px-4 py-3 text-sm text-emerald-400 hover:bg-slate-700 font-bold border-t border-slate-700/50">🖨️
                            Print as PDF</button>
                    </div>
                </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
            <div
                class="bg-slate-800 border border-slate-700 p-6 rounded-2xl shadow-sm hover:border-indigo-500/50 transition-colors">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
            ...
            <div class="relative flex-1 md:max-w-md">
                <input type="text" id="ghostSearch" placeholder="Search notes, categories, authors..."
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-slate-800/50 border-b border-slate-700">
                            <th class="py-4 px-6 text-xs font-semibold uppercase tracking-wider text-slate-400">Date
                            </th>
                            <th class="py-4 px-6 text-xs font-semibold uppercase tracking-wider text-slate-400">Priority
                            </th>
                            <th class="py-4 px-6 text-xs font-semibold uppercase tracking-wider text-slate-400">Category
                            </th>
                            <th class="py-4 px-6 text-xs font-semibold uppercase tracking-wider text-slate-400">Tag</th>
                            <th class="py-4 px-6 text-xs font-semibold uppercase tracking-wider text-slate-400">Author
                            </th>
                            <th class="py-4 px-6 text-xs font-semibold uppercase tracking-wider text-slate-400">Context
                            </th>
                            <th class="py-4 px-6 text-xs font-semibold uppercase tracking-wider text-slate-400">Message
                            </th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-800">
                        @forelse($rows as $row)
                            <tr class="group hover:bg-slate-800/30 transition-all">
                                <td class="py-4 px-6 text-sm text-slate-400 font-mono whitespace-nowrap">
                                    {{ $row['date'] }}</td>
                                <td class="py-4 px-6">
                                    <span
                                        class="px-2 py-0.5 rounded text-[10px] font-bold border {{ $row['priority'] == 'HIGH' ? 'bg-red-500/10 text-red-400 border-red-500/30' : ($row['priority'] == 'MEDIUM' ? 'bg-amber-500/10 text-amber-400 border-amber-500/30' : 'bg-slate-700/50 text-slate-400 border-slate-600/50') }}">
                                        {{ $row['priority'] }}
                                    </span>
                                </td>
                                <td class="py-4 px-6">
                                    @php
                                        $typeStyles = [
                                            ...
                                            'NOTE' => 'bg-cyan-500/10 text-cyan-400 border-cyan-500/20',
                                            'GENERAL' => 'bg-slate-500/10 text-slate-400 border-slate-500/20',
                                        ];
                                        $style = $typeStyles[$row['type'] ?? 'GENERAL'] ?? $typeStyles['GENERAL'];
                                    @endphp
                                    <span
                                        class="px-2 py-0.5 rounded text-[10px] font-bold border {{ $style }} uppercase">
                                        {{ $row['type'] ?? 'GENERAL' }}
                                    </span>
                                </td>
                                <td class="py-4 px-6">
                                    <span
                                        class="px-2 py-0.5 rounded text-[10px] font-bold bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 uppercase">
                                        {{ $row['tag'] }}
                                    </span>
                                </td>
                                <td class="py-4 px-6 text-sm text-slate-300 whitespace-nowrap">{{ $row['author'] }}
                                </td>
                                <td class="py-4 px-6">
                                    <div class="flex flex-col gap-1">
                                        <a href="{{ $row['vscode'] }}"
                                            class="text-[10px] text-blue-400 hover:underline flex items-center gap-1">🖥️
                                            VS Code</a>
                                        <a href="{{ $row['link'] }}" target="_blank"
                                            class="text-[10px] text-slate-500 hover:text-indigo-400 font-mono truncate max-w-[120px]">{{ $row['file'] }}</a>
                                    </div>
                                </td>
                            ...
                            <tr class="bg-black/10">
                                <td colspan="7" class="px-6 py-2">
                                    <details class="group">
                                        <summary
                                            class="text-[10px] text-slate-600 group-hover:text-slate-400 cursor-pointer uppercase tracking-widest font-bold select-none">
                                            View Source</summary>
                                        <pre
                                            class="mt-2 p-4 bg-[#050a15] rounded-lg text-xs font-mono text-emerald-400/90 overflow-x-auto border border-slate-800"><code>{{ base64_decode($row['snippet']) }}</code></pre>
                                    </details>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="7" class="py-20 text-center text-slate-500 italic">No ghost notes found.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
        ...
        <div id="resolved-table"
            class="hidden bg-slate-900 border border-slate-800 shadow-2xl rounded-3xl overflow-hidden p-8 text-center">
            <h2 class="text-xl font-bold text-emerald-400 mb-6">🏆 Resolved Technical Debt</h2>
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr
                        class="bg-slate-800/50 border-b border-slate-700 text-slate-400 text-[10px] uppercase tracking-wider">
                ...
                <tbody>
                    @forelse ($history as $item)
                        <tr class="border-b border-slate-800 text-sm text-slate-300 hover:bg-slate-800/20">
                            ...
                            <td class="py-4 px-6"><span class="text-indigo-400 font-bold">#{{ $item['tag'] }}</span>
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="py-12 text-slate-600">No resolved ghosts yet. Keep hacking!</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <script>
        ...
        function switchTab(tab) {
            ...
            if (tab === 'active') {
                ...
                btnActive.className =
                    "py-2.5 px-6 rounded-lg bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 font-bold transition-all";
                btnResolved.className =
                    "py-2.5 px-6 rounded-lg text-slate-500 hover:text-slate-300 font-bold transition-all";
            } else {
                activeTable.classList.add('hidden');
                resolvedTable.classList.remove('hidden');
                btnResolved.className =
                    "py-2.5 px-6 rounded-lg bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 font-bold transition-all";
